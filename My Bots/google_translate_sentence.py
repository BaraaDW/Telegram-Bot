...
def cancel(update, context):
    update.message.reply_text('Bye! I hope we can talk again some day.\n if you want start again write /start.', reply_markup=ReplyKeyboardRemove())
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ENGLISH: [CommandHandler('cancel', cancel), MessageHandler(Filters.text, english_to_arabic)],  # 'Filters.text' include (text) and (command), that why i put the 'CommandHandler' first [-DW-]
            ARABIC: [CommandHandler('cancel', cancel), MessageHandler(Filters.text, arabic_to_english)]  # 'Filters.text' include (text) and (command), that why i put the 'CommandHandler' first [-DW-]
        },
